package io.taucoin.torrent.publishing.core.utils.selecttext;
...
//            Log.e("SelectableTextHelper", "start: " + mSelectionInfo.mStart + "   end: " + mSelectionInfo.mEnd);
    ...
    private class OperateWindow {
        ...
        OperateWindow(final Context context) {
            ...
            mWidth = rv_content.getPaddingStart() * 4 + dp2px(itemWidth * (Math.min(size, mPopSpanCount)));
            // 行数
...
//            mHeight = dp2px(12 * (1 + row) + 52 * row + 5);
    ...
    public static int getStatusHeight() {
        ...
        if (resid > 0) {
            STATUS_HEIGHT = getResources().getDimensionPixelSize(resid);
            return STATUS_HEIGHT;
        }
        return -1;
    }
