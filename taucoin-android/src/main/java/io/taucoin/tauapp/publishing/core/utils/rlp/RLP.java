 ...
 */
package io.taucoin.tauapp.publishing.core.utils.rlp;
...
import static io.taucoin.tauapp.publishing.core.utils.rlp.ByteUtil.*;
...
public class RLP {
    ...
    public static int decodeInt(byte[] data, int index) {
	    ...
	    } else if ((data[index] & 0xFF) <= OFFSET_SHORT_ITEM + Integer.SIZE / Byte.SIZE) {
    ...
    static short decodeShort(byte[] data, int index) {
    	...
    	} else if ((data[index] & 0xFF) <= OFFSET_SHORT_ITEM + Short.SIZE / Byte.SIZE) {
    ...
    public static long decodeLong(byte[] data, int index) {
    	...
    	} else if ((data[index] & 0xFF) <= OFFSET_SHORT_ITEM + Long.SIZE / Byte.SIZE) {
